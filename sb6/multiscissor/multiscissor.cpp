...
const char *gs_source = {
    ...
    "layout (std140, binding = 0) uniform UB_MODELVIEW                                      \n"
    "{                                                                                      \n"
    ...
    "        if (gl_ViewportIndex == 0 ) gs_out.color *= 0.1;                               \n"
    "        EmitVertex();                                                                  \n"
    "    }                                                                                  \n"
    "    EndPrimitive();                                                                    \n"
    "}                                                                                      \n"
...
void App::render()
{
	...
	{
		Attrs attrs = {
		        {"viewport1", viewport(0)},
                        {"scissor1",  scissors[0]},
                        {"viewport2", viewport(0)},
		        {"scissor2",  scissors[1]},
                        {"viewport3", viewport(0)},
                        {"scissor3",  scissors[2]},
		        {"viewport4", viewport(0)},
                        {"scissor4",  scissors[3]},
		};
		BaseApp::set(attrs);
	}
	// draws
	{
		...
		for (auto i = 0; i < 5; i++) {
			ub_modelview[i]
			        = u_viewscreen * c_unit.rot("XY", t * 81.0, t * 45.0).trans({0.0, 0.0, -2.0});
		}
	...
	auto &renderstate = getRenderstate();
	renderstate.get();  // sync scissor test
}

static ObjectRegistry<SpuPage>::Creator<App> spu_sketch_creator("multiscissor");
}  // namespace spu::multiscissor
