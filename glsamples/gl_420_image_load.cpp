//
// App :
//
#include "base_app.h"
namespace spu {
namespace {

/* clang-format off */
const char *c_vert = {
    "#version 420 core                                                                      \n"
    "uniform mat4 u_worldscreen;                                                             \n"
    "in vec2 a_position;                                                                    \n"
    "in vec2 a_texcoord;                                                                    \n"
    "out gl_PerVertex                                                                       \n"
    "{                                                                                      \n"
    "    vec4 gl_Position;                                                                  \n"
    "};                                                                                     \n"
    "out vec2 f_texcoord;                                                                   \n"
    "void main()                                                                            \n"
    "{                                                                                      \n"
    "    f_texcoord = a_texcoord;                                                           \n"
    "    gl_Position = u_worldscreen * vec4(a_position, 0.0, 1.0);                           \n"
    "}                                                                                      \n"
};

const char *c_frag = {
    "#version 420 core                                                                      \n"
    "layout(binding = 0, rgba8) coherent uniform image2D u_diffuse;                         \n"
    "uniform ivec2 u_image_size;                                                            \n"
    "in vec2 f_texcoord;                                                                    \n"
    "layout(location = 0, index = 0) out vec4 color;                                        \n"
    "void main()                                                                            \n"
    "{                                                                                      \n"
    "    color = imageLoad(u_diffuse, ivec2(f_texcoord * u_image_size));                    \n"
    "}                                                                                      \n"
};

/* clang-format on */
class App : public BaseApp {
public:
	SpuShader m_shader;
	Vec4i u_image_size;
	uint32_t m_arrayId;

	Mat4f u_worldscreen;
	uint32_t u_diffuse;

	App(const char *name) : BaseApp(name, true, c_orange) {}

	void init(const Attrs &attrs) override
	{
		BaseApp::init(attrs);

		// texture
		{
			auto width = 0;
			auto height = 0;
			Attrs attrs = {
			        {"iformat",    GL_RGBA8  }, // no srgb
			        {"pformat",    GL_BGRA   },
			        {"mag_filter", GL_NEAREST},
			        {"min_filter", GL_NEAREST},
			};
			u_diffuse = loadDDS("kueken7_bgra8_srgb.dds", attrs);
			spu_texture_get(u_diffuse, "width", &width);
			spu_texture_get(u_diffuse, "height", &height);
			u_image_size = Vec4i(width, height, 0, 0);
		}
		// program
		{
			Attrs shader_attrs = {
			        {"vert", c_vert},
			        {"frag", c_frag},
			};
			Attrs unif_attrs = {
			        {"u_worldscreen", &u_worldscreen},
			        {"u_image_size",  &u_image_size },
			        {"u_diffuse",     &u_diffuse    },
			};
			loadShader(m_shader, shader_attrs, unif_attrs);

			auto access = GL_READ_ONLY;
			spu_shader_set(m_shader.id(), "u_diffuse.access", &access);
		}
		// array #0
		{
			Attrs attrs = {
			        {"shader_id",    m_shader.id()},
			        {"a.a_position", 2            },
			        {"a.a_texcoord", 2            },
			};
			m_arrayId = squareQuadsArray<v2fv2f_t>(attrs);
		}
	}

	void render() override
	{
		getCamera().setViewscreen(viewport(0), 45, 0.1, 1000.0);

		u_worldscreen = getCamera().worldscreen();
		m_shader.use();
		spu_array_draw(m_arrayId, GL_TRIANGLES);
	}
};

static ObjectRegistry<SpuPage>::Creator<App> spu_sketch_creator("gl_420_image_load");
}  // namespace
}  // namespace spu
