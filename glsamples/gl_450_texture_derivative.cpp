...
const char *c_frag_x = {
    ...
    "float textureLevel(in sampler2D u_sampler, const vec2 texcoord)                        \n"
    "{                                                                                      \n"
    "    vec2 texture_size = vec2(textureSize(u_sampler, 0));                               \n"
    "    float levelCount = max(log2(texture_size.x), log2(texture_size.y));                \n"
    "    vec2 dx = dFdxCoarse(texcoord * texture_size);                                     \n"
    "    vec2 dy = dFdyCoarse(texcoord * texture_size);                                     \n"
...
static ObjectRegistry<SpuPage>::Creator<App> spu_sketch_creator("gl_450_texture_derivative");
}  // namespace
}  // namespace spu
