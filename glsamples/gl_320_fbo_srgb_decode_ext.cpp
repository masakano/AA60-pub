...
public:
	...
	void render() override
	{
...

		{
			...
			// m_textureIds[0] is a sRGB texture which sRGB conversion
			// on fetch has been disabled
			// Hence in the shader, the value is stored as sRGB so we should not
			// convert it to sRGB.

			u_diffuse = m_textureIds[0];
			m_shaders[0].use();

			spu_array_draw(m_arrayIds[0], GL_TRIANGLES, 0, 0, 2);  // wny 2 times?
...
static ObjectRegistry<SpuPage>::Creator<App> spu_sketch_creator("gl_320_fbo_srgb_decode_ext");
}  // namespace
}  // namespace spu
