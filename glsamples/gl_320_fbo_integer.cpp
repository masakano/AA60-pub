...
public:
	...
	void init(const Attrs &attrs) override
	{
		...
		const float fb_scale = 4;
		auto fb_viewport = Rectf(0, 0, viewport(0).sx / fb_scale, viewport(0).sy / fb_scale);
		...
		// array
		{
			...
			spu_array_send(m_arrayIds[0], c_vertices.data(), c_vertices.size(), 0);
		...
		// diffuse texture
		{
			Attrs attrs = {
			        {"iformat",     GL_RGB8UI       },
			        {"pformat",     GL_RGB_INTEGER  }, // explicit pixel format!
			        {"max_level",   0               },
		...
		// frame buffer
		{
			auto bgcolor0 = Vec4f(0, 128, 255, 255);
			Attrs attrs = {
			        {"color0",    m_textureIds[1]},
			        {"viewport0", fb_viewport    },
			        {"bgcolor0",  bgcolor0       }, // tricky, but works
			};
...
static ObjectRegistry<SpuPage>::Creator<App> spu_sketch_creator("gl_320_fbo_integer");
}  // namespace
}  // namespace spu
