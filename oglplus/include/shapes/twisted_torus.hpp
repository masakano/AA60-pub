...
class TwistedTorus : public Shape {
private:
	const double m_radius_out{1.0}, m_radius_in{0.5}, m_thickness{0.02};
	const double /*m_r_slip_coef{ 0.25 },*/ m_s_slip_coef{0.20};
	const uint32_t m_sections{12}, m_rings{48}, m_twist{12};
...
public:
	...
	TwistedTorus(
	        double rad_out, double rad_in, double thickness, uint32_t sects, uint32_t rings, uint32_t twist)
	        : m_radius_out(rad_out), m_radius_in(rad_in), m_thickness(thickness), m_sections(sects),
	          m_rings(rings), m_twist(twist)
	{
...
#include <shapes/twisted_torus.ipp>
