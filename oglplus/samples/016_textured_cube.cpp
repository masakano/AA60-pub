...
public:
	...
	void init(const Attrs &attrs) override
	{
		...
		{
			images::Image image = images::LoadTexture("concrete_block");
	...
	void render() override
	{
		...
		u_viewsceen = math::perspective(viewport(0), 90, 1, 30);
		u_worldview = Mat4f::orbiting(ezero(), esec, 5.5, -3.0, 6.0, 0, 1.42, 0, 90, 30);
		u_nodeworld = math::unit().rot("x", -esec / 10 * math::two_pi());
		m_array.draw(nullptr);
	}
};

static ObjectRegistry<SpuPage>::Creator<App> sandbox_creator("016_textured_cube");
}  // namespace
}  // namespace spu::oglplus
