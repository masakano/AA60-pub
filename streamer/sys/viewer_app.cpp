//
// ReceiverTweakbar :
//
#include "base_app.h"
#include <gsys/canvas/copy.h>
#include <ssys/shared_memory.h>

namespace spu {

class ViewerApp : public BaseApp {
public:
	explicit ViewerApp(const char *name) : BaseApp(name) {}
	void init(const Attrs &attrs) override;
	void render() override;

private:
	SharedMemory m_shm;
	gs_canvas::Copy m_copy;
	SpuTexture m_texture;
};

void ViewerApp::init(const Attrs &attrs)
{
	GsPage::init(attrs);

	Attrs texture_attrs = {
	        {"iformat",     GL_SRGB8_ALPHA8},
                {"width",       c_width        },
                {"height",      c_height       },
                {"max_level",   0              },
	        {"auto_mipmap", 0              },
	};
	m_texture.init(texture_attrs);

	Attrs canvas_attrs = {
	        {"use_unif_block", false         },
	        {"def_vflip",      0             },
	        {"path",           "flip_copy.us"},
	};
	m_copy.init(canvas_attrs);
	m_copy.u_color0 = m_texture.id();
	m_shm.init(c_shm_path, sizeof(SharedBuffer) + c_width * c_height * sizeof(RGBA8));
}

void ViewerApp::render()
{
	auto *ptr = (SharedBuffer *)m_shm.ptr();
	spu_printf(
	        0, "count = %08x, pixels = %02x,%02x,%02x,%02x\n", ptr->count, ptr->pixels[0].r,
	        ptr->pixels[0].g, ptr->pixels[0].b, ptr->pixels[0].a);

	m_texture.send(ptr->pixels, GL_SRGB8_ALPHA8);
	m_copy.render();
}
static ObjectRegistry<GsCanvas>::Creator<ViewerApp> page_creator("viewer");
}  // namespace spu
